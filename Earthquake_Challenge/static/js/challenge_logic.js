...
// Create the 2nd layer group for the tectonic plate data.
let tectonicPlates = new L.LayerGroup();

//  Craete the 3rd layer group for the major earthquakes.
...
// This function returns the style data for each of the major earthquakes we plot on
  // the map. We pass the magnitude of the tectonic into two separate functions
  // to calculate the color and radius.
  function styleInfo(feature) {
    return {
      opacity: 1,
  ...
  // This function determines the color of the marker based on the magnitude of the major earthquakes.
  function getColor(magnitude) {
    switch (true) {
    case (magnitude > 5):
      return "red";
...
//   // This function determines the radius of the major earthquakes marker based on its magnitude.
//   // Earthquakes with a magnitude of 0 were being plotted with the wrong radius.
  function getRadius(magnitude) {
    if (magnitude === 0) {
      return 1;
  ...
  L.geoJson(data, {
      ...
      // We set the style for each circleMarker using our styleInfo function.
    style: styleInfo,
     // We create a popup for each circleMarker to display the layer and name of the major earthquakes
     //  after the marker has been created and styled.
     onEachFeature: function(feature, layer) {
      layer.bindPopup("<h3> Magnitude: " + feature.properties.mag + "<br>Location: " + feature.properties.place + "</h3>");
    }
...
// Looping through our intervals to generate a label with a colored square for each interval.
  ...
  // Create a style for the lines of tectonic plate data.
  let myStyle = {
    color: "red",
    weight: 1.5
};
...
d3.json(TectonicLink).then(function(tectonicdata) {
